...
[[ "$DOCROOT" == "/home/belastock-loja/htdocs/loja.belastock.com.br" ]] || fail "Trava de diretório acionada."

# Travas contra qualquer operação no site principal.
FORBIDDEN=(
...
for p in "${EXISTING_PATHS[@]:-}"; do
  [[ -z "$p" || "$p" == "$DOCROOT" ]] || fail "O domínio já existe em outro usuário: $p"
done
...
if [[ ! -f "$SECRETS_FILE" ]]; then
  log "Gerando credenciais locais (não entram no GitHub)"
  umask 077
  SITE_USER_PASSWORD="$(openssl rand -hex 18)"
...
if [[ ! -d "$DOCROOT" ]]; then
  ...
  TEMPLATE="Generic"
  if clpctl vhost-templates:list 2>/dev/null | grep -qi "WordPress"; then TEMPLATE="WordPress"; fi
  clpctl site:add:php \
    --domainName="$DOMAIN" \
    --phpVersion=8.4 \
    --vhostTemplate="$TEMPLATE" \
    --siteUser="$SITE_USER" \
    --siteUserPassword="$SITE_USER_PASSWORD"
fi

[[ -d "$DOCROOT" ]] || fail "CloudPanel não criou o diretório esperado: $DOCROOT"
...
log "Sincronizando somente tema e plugin próprios"
...
log "Instalando WooCommerce e integrações oficiais/principais"
...
# Remover conteúdo padrão, sem tocar em produtos do usuário.
...
if clpctl lets-encrypt:install:certificate --domainName="$DOMAIN" >/tmp/belastock-loja-ssl.log 2>&1; then
  ...
  log "SSL ainda não emitido; normalmente isso significa DNS ainda não apontado."
fi
