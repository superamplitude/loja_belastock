...
function belastock_ensure_page(string $title, string $slug, string $content): int {
    ...
    $placeholder_markers = [
        ...
        'Esta página apresenta as regras',
        'Esta pagina apresenta as regras',
        'Criações para vestir, usar e levar com você.',
        'Criacoes para vestir, usar e levar com voce.',
    ];
    $should_update = trim((string) $page->post_content) === '';
    foreach ($placeholder_markers as $marker) {
        if (strpos((string) $page->post_content, $marker) !== false) {
            $should_update = true;
            break;
        }
    }
...
function belastock_ensure_attribute(string $name, string $slug, array $terms): void {
    ...
    foreach ((array) wc_get_attribute_taxonomies() as $attribute) {
        if ((string) $attribute->attribute_name === $slug) {
            $found = true;
            break;
        }
    }
...
$about = belastock_ensure_page(
    'Sobre a Bela Stock',
    'sobre-a-bela-stock',
    '<h2>Bela Stock</h2><p>A Bela Stock reúne camisetas, bonés e adesivos com identidade própria. A loja foi preparada para apresentar cada peça em vários ângulos, incluindo frente, verso, lateral, detalhes e mockups reais.</p><p>Os produtos, variações, disponibilidade e prazos apresentados em cada página prevalecem no momento da compra.</p>'
);

$shipping = belastock_ensure_page(
    'Entregas e frete',
    'entregas-e-frete',
    '<h2>Entregas e frete</h2><p>O frete é calculado a partir do CEP informado no carrinho ou no checkout, conforme os serviços de entrega habilitados para o destino. Quando disponíveis, serão apresentadas as modalidades, valores e estimativas de prazo antes da confirmação do pedido.</p><p>Após a postagem, o acompanhamento será disponibilizado conforme o serviço de transporte utilizado. Confira cuidadosamente o endereço antes de concluir a compra.</p>'
);
...
$privacy = belastock_ensure_page(
    'Política de privacidade',
    'politica-de-privacidade',
    '<h2>Política de privacidade</h2><p>Os dados informados na loja são utilizados para processar pedidos, pagamentos, entregas, atendimento, prevenção a fraudes e cumprimento de obrigações legais. Dados de pagamento podem ser processados diretamente pelos provedores de pagamento habilitados no checkout.</p><p>Informações necessárias à entrega podem ser compartilhadas com operadores logísticos e transportadores. A Bela Stock adota medidas técnicas e administrativas compatíveis com a proteção dos dados e limita o tratamento às finalidades relacionadas à operação da loja.</p><p>Para assuntos relacionados a dados pessoais, utilize o canal de contato informado neste site.</p>'
);

$terms = belastock_ensure_page(
    'Termos e condições',
    'termos-e-condicoes',
    '<h2>Termos e condições</h2><p>Ao realizar um pedido, o cliente confirma os produtos, quantidades, variações, endereço, forma de entrega e forma de pagamento apresentados no checkout. O pedido fica sujeito à confirmação do pagamento e à disponibilidade indicada pela loja.</p><p>Preços, promoções e condições válidas são os exibidos no momento da conclusão da compra. Em produtos personalizados, a produção poderá depender da confirmação das características ou aprovação da arte correspondente.</p><p>Estes termos não restringem direitos garantidos pela legislação brasileira de proteção ao consumidor.</p>'
);

$contact = belastock_ensure_page(
    'Contato',
    'contato',
    '<h2>Fale com a Bela Stock</h2><p>Para dúvidas sobre produtos, pedidos, entregas, trocas ou pagamentos, envie um e-mail para <a href="mailto:contato@belastock.com.br">contato@belastock.com.br</a>. Ao falar sobre uma compra, informe o número do pedido para agilizar o atendimento.</p>'
);
...
belastock_ensure_attribute('Cor', 'cor', ['Preto', 'Branco', 'Cinza', 'Azul-marinho']);
